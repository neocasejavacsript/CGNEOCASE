...
Description - Field “Last Working Day” will be visible and mandatory only when reason = Salary Continuance
            - Mandatory SAP Hiring Form for SCFI_Landed Inbound Start (with custom JS due to discrepancy of core Neocase mandatory , noMandatory)
---------------------------------------------------------*/ 
...
/*---- Mandatory SAP Hiring Form for SCFI_Landed Inbound Start --++MOD-012--*/
window.mandateSAPFormField = function(){
    var subtopic = neocase.form.field("INTERVENTIONS_EN_COURS$ELEMENT").getValue() || getParamFromUrl('subtopic'),
        hiringFormLabel = $.trim($('#section5dfe1d12ce0e18a0433b').find('.row label').text().split(':')[0]);
    if(subtopic == '2909'){
        neocase.form.field('INTERVENTIONS_EN_COURS$VALEUR525').mandatory('Hiring Form');
        if($('#section5dfe1d12ce0e18a0433b').find('.ValidatorCautionBox').length< 1){
            $('#section5dfe1d12ce0e18a0433b').find('.fileupload').append("<span class='ValidatorCautionBox'></span>");
            $($('#section5dfe1d12ce0e18a0433b').find('.btn').children()[2]).attr('neo-required-message', hiringFormLabel);
        }
    }   
    // else{
    //     console.log("Hello No mandatory");
    //     neocase.form.field('INTERVENTIONS_EN_COURS$VALEUR525').noMandatory();
    // }
};
$($('#section5dfe1d12ce0e18a0433b').find('.btn').children()[2]).on('change', function () {     
    var subtopic = neocase.form.field("INTERVENTIONS_EN_COURS$ELEMENT").getValue() || getParamFromUrl('subtopic'),
        hiringFormLabel = $.trim($('#section5dfe1d12ce0e18a0433b').find('.row label').text().split(':')[0]);
    if(subtopic == '2909'){
        if($($('#section5dfe1d12ce0e18a0433b').find('.btn').children()[2]).val() != '' && $('#section5dfe1d12ce0e18a0433b').find('.ValidatorCautionBox').length > 0){
            console.log('hello'+$('.fileinput-display').children().length);
            $('#section5dfe1d12ce0e18a0433b').find('.ValidatorCautionBox').remove();
            $($('#section5dfe1d12ce0e18a0433b').find('.btn').children()[2]).removeAttr('neo-required-message');
        }
        else{
            if($('#section5dfe1d12ce0e18a0433b').find('.ValidatorCautionBox').length< 1){
                $('#section5dfe1d12ce0e18a0433b').find('.fileupload').append("<span class='ValidatorCautionBox'></span>");
...
/*--XXX-- Mandatory SAP Hiring Form for SCFI_Landed Inbound Start --XXX--*/ 
