...
window.generateDocumentButtonDisplay = function(){
    var getSubtopicValue = neocase.form.field('INTERVENTIONS_EN_COURS$ELEMENT').getText(),
        elementGenerateButtonHRBP = document.getElementById('bouton_evenement3569');
        elementGenerateButtonMGR = document.getElementById('bouton_evenement3628');
        elementGenerateButtonEE = document.getElementById('bouton_evenement3627');
        
    console.log(getSubtopicValue);
    try{
        if(elementGenerateButtonHRBP !== null){
            if(getSubtopicValue == 'LC_Employment Verification Letter (HRBP)'){
                elementGenerateButtonHRBP.style.display = 'block';
            }else{
                elementGenerateButtonHRBP.style.display = 'none';
            }
        }
        if(elementGenerateButtonMGR !== null){
            if(getSubtopicValue == 'LC_Employment Verification Letter (MGR)'){
                elementGenerateButtonMGR.style.display = 'block';
            }else{
                elementGenerateButtonMGR.style.display = 'none';
            }
        }
        if(elementGenerateButtonEE !== null){
            if(getSubtopicValue == 'LC_Employment Verification Letter (EE)'){
                elementGenerateButtonEE.style.display = 'block';
            }else{
                elementGenerateButtonEE.style.display = 'none';
            }
        }
    }catch(error){
        console.log(error);
...
window.hideAnnualSalOnEE = function(subtopic){
    if(subtopic=="LC_Change In Working Hours (EE)"){
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR75").hide();
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR872").hide();
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR384").hide();
    }else{
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR75").show();
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR872").show();
        neocase.form.field("INTERVENTIONS_EN_COURS$VALEUR384").show();
    }
};
/*************************
...
window.onloadForm = function() {
    ...
    setTimeout(function(){
        ...
        hideAnnualSalOnEE(getSubtopicValue);
